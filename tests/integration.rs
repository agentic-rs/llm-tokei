...
fn codex_fixture_parses_last_total() {
    ...
    // 120*1.25 + 90*10 + 20*10 (reasoning falls back to output) + 80*0.125 = 150+900+200+10 = 1260
    // / 1e6 = 0.00126
    let base = row["cost_base"].as_f64().unwrap();
    assert!((base - 0.00126).abs() < 1e-9, "got {base}");
    // openai provider has multiplier 1.0 → cost_multiplied == cost_base
